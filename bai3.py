...
def thongBaoChoAdmin(tenKhach, emailKhach, ngay, date, suc_chua, ly_do):
    anfitrion = "smtp.gmail.com"
    puerto = 587
    diaChiNguoiGui = "luccc@gmail.com"
    matKhauNguoiGui = "yourpassword"
    diaChiAdmin = "luccc@gmail.com"  # Địa chỉ admin
...
valid_emails = ["luc.2274802010521@vanlanguni.vn", "duy.2274802010146@vanlanguni.vn"]
@app.route('/')
def home():
...
def search():
    if request.method == 'POST':
        ...
        query = "SELECT * FROM university.phong WHERE 1=1"
        if suc_chua:
...
def validar(id):
	if request.method=='POST':
		ma = request.form['ma'] 
		cursor = db.connection.cursor()
		contador = cursor.execute("SELECT * FROM user WHERE ma = '{0}'".format(ma))
		data=cursor.fetchall()
		if contador>0:
			mail = data[0]
			global email
			...
			return render_template('home.html',id=id,ola=1)
